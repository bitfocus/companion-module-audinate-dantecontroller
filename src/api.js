...
export default {
	...
	updateChannelChoices: function (deviceIp, channelType) {
		...
		if (!this[channelType + 'ChannelsChoices'][deviceName]) {
			...
			for (let i = 1; i < channelChoice.length; i++) {
				if (
					!this[channelType + 'ChannelsChoices'][deviceName][i] ||
					channelChoice[i].label != this[channelType + 'ChannelsChoices'][deviceName][i].label
				) {
					this[channelType + 'ChannelsChoices'][deviceName] = channelChoice
					this.updateData()
					break
				...
				case DANTE_CONST.COMMANDS.MESSAGE_TYPE_ENCODING_STATUS: {
					...
					if (optionsNumber && optionsNumber > 0) {
						...
						if (
							...
							!compareArrays(currDevice.encodingOptions, this.devicesData[deviceIp]?.encodingOptions)
						) {
				...
				case DANTE_CONST.COMMANDS.MESSAGE_TYPE_SAMPLE_RATE_STATUS: {
					...
					if (optionsNumber && optionsNumber > 0) {
						...
						if (
							...
							!compareArrays(currDevice.srOptions, this.devicesData[deviceIp]?.srOptions)
						) {
				...
				case DANTE_CONST.COMMANDS.MESSAGE_TYPE_SAMPLE_RATE_PULLUP_STATUS: {
					...
					if (optionsNumber && optionsNumber > 0) {
						...
						if (
							...
							!compareArrays(currDevice.pullupOptions, this.devicesData[deviceIp]?.pullupOptions)
						) {
				...
				case DANTE_CONST.COMMANDS.MESSAGE_TYPE_CODEC_STATUS: {
					...
					for (let i = 0; i < channelCount; i++) {
						...
						if (
							...
							!compareArrays(currDevice.output_levels, this.devicesData[deviceIp]?.output_levels)
						) {
	...
	sendCommand(command, host, service = 'ARC', forcePort) {
		...
		if (port) {
			...
			this.log('error', `Undefined port for service ${service} for device ${deviceId}`)
			return
		}
	},
	...
	makeCrosspoint(destinationDevice, sourceChannelName, sourceDeviceName, destinationChannel) {
		...
		const destinationChannelNumber =
			this.findRxChannelByName(destinationDevice, destinationChannel)?.number ?? destinationChannel
	...
	getTxChannelFriendlyNames(ipaddress) {
		...
		for (let page = 0; page <= Math.ceil(this.devicesData[ipaddress]?.tx?.count / 32); page++) {
			...
			const commandBuffer = this.makeCommand(
	...
	setSampleRate(ipaddress, sampleRate) {
		...
		const commandBuffer = this.makeSettingCommand(
	...
	setLevel(ipaddress, _direction = 'out', channelNumber, levelSetting) {
		const commandArguments = Buffer.concat([
