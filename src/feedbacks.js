			...
			callback: (feedback) => {
				...
				if (opt.destinationDevice && self.devicesData[opt.destinationDevice]?.rx && opt.sourceDevice) {
					...
					const sourceChannel =
						...
						self.findTxChannelByName(opt.sourceDevice, selectedSourceChannel)
		...
		feedbacks['routing_bg_manual'] = {
			...
			callback: async function (feedback, context) {
				...
				const destinationDeviceIp = IP.test(destinationDeviceId)
					? destinationDeviceId
					: self.findDeviceIpByName(destinationDeviceId)

				if (destinationDeviceIp && sourceDeviceName && self.devicesData[destinationDeviceIp]?.rx) {
					const destinationChannel =
						self.findRxChannelByName(destinationDeviceIp, destinationChannelId) ??
						self.devicesData[destinationDeviceIp].rx[destinationChannelId]
